...
PATTERNS='agent:[a-z0-9_-]+:(telegram|imessage|discord|whatsapp|signal|webchat):|telegram:direct:[0-9]|(/Users/|/home/)[a-z0-9_-]+/|~/\.(openclaw|coven)/(agents|workspaces|credentials|sessions)|\+1[0-9]{10}'
if [ "$MODE" = "--staged" ]; then
...
if [ "${2:-}" = "--beads" ] || [ "$MODE" = "--beads" ]; then
  if command -v bd >/dev/null 2>&1; then
    TMP=$(mktemp)
    bd export -o "$TMP" >/dev/null 2>&1 || true
    if [ -s "$TMP" ] && grep -EqI "$PATTERNS" "$TMP"; then
